#lang racket
(struct prog (id disc children) #:transparent)

(define example-input "pbga (66)
xhth (57)
ebii (61)
havc (66)
ktlj (57)
...
cntj (57)")
(define (main1 inp)
  (let* ([progs (map parseline (string-split inp "\n"))]
         [children-set (apply set (flatten (map prog-children progs)))])
    (filter (λ (p) (not (set-member? children-set (prog-id p)))) progs)))

(define (main2 inp [start #f])
  (let* ([progs (map parseline (string-split inp "\n"))]
         [children-set (apply set (flatten (map prog-children progs)))]
         [graph (make-hash (map (λ (p) (cons (prog-id p) p)) progs))]
         [tree-base (if start (hash-ref graph start) (first (filter (λ (p) (not (set-member? children-set (prog-id p)))) progs)))])
    (main2* tree-base graph)))

(define (main2* tree-base graph)
  (let* ([node-weights (get-node-weight (prog-id tree-base) graph)]
        [bad-ref (check-weights* node-weights)])
    (print tree-base)
    (println node-weights)
    (println "")
    (if bad-ref
        (main2* (hash-ref graph (list-ref (prog-children tree-base) bad-ref)) graph)
        tree-base)))

(define (sum l)
  (foldl + 0 l))

(define (check-weights* res)
  (if
   (< 1 (length res))
   (let ([rez (get-rez-sums res)])
     (print rez)
   (if (apply = rez)
       #f
       (if (= 1 (length (indexes-of rez (apply min rez))))
           (first (indexes-of rez (apply min rez)))
           (first (indexes-of rez (apply max rez))))))
   #f))

#;(You are a dummy and didn't realize this wanted the new weight)
(define (get-rez-sums res)
  (map (λ (l) (sum (flatten l))) (list-tail res 1)))

(define (get-node-weight node graph)
  (let ([prog (hash-ref graph node)])
  (if
   (= 0 (length (prog-children prog)))
   (list (prog-disc prog))
   (cons (prog-disc prog) (map (λ (c) (get-node-weight c graph)) (prog-children prog))))))

(define (parseline line)
  (let ([splitline (string-split line)])
    (cond
      [(= 2 (length splitline)) (first splitline)
                                (prog
                                 (first splitline)
                                 (parse-weight (second splitline))
                                 '())]
      [else (prog
             (first splitline)
             (parse-weight (second splitline))
             (map (λ (s) (string-trim s ",")) (list-tail splitline 3)))])))
(define (parse-weight s)
  (string->number (string-trim (string-trim s ")") "(")))
(main1 example-input)
(main2 example-input)


(define puzz-input "tqefb (40)
