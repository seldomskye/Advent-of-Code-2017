#lang racket
(define input "nyot babgr babgr kqtu kqtu kzshonp ylyk psqk
...
(define (pass-splitter input)
  (map (λ (inp) (map sort-word (string-split inp " "))) (string-split input "\n")))
(define input* (pass-splitter input))

(define (part1 split-pass)
  (= (length split-pass) (set-count (apply set split-pass))))

(length (filter values (map part1 input*)))