...
(define example-input "b inc 5 if a > 1
a inc 1 if b < 5
c dec -10 if a >= 1
c inc -20 if c == 10")

(define-struct line (reg op cond) #:transparent)
(define-struct condit (reg cmp)#:transparent)

(define (part1 inp-string)
  (let ([regs (make-hash)]
        [highest-value (box 0)])
  (foldl max 0 (hash-values (foldl (execute highest-value) regs (parse-input inp-string))))
    (unbox highest-value)))

(define (parse-input raw)
  (map parse-line (map (λ (s) (string-split s " ")) (string-split raw "\n"))))

(define (parse-line l)
  (let ([condit (make-condit (fifth l) (λ (v) ((get-cmp (sixth l)) v (string->number (seventh l)))))]
        [op (λ (v) ((get-op (second l)) v (string->number (third l))))]
        )
    (make-line (first l) op condit)))
...
(define (get-cmp s)
  (cond
    ...
    [(equal? s ">=") >=]
    [(equal? s "<=") <=]
    [(equal? s "!=") (λ (v s) (not (= v s)))]))

(define (execute box)
  (λ (line regs)
  (let ([condit (line-cond line)])
    (if
     ((condit-cmp condit) (hash-ref regs (condit-reg condit) 0))
     (begin
       (let ([next-val ((line-op line) (hash-ref regs (line-reg line) 0))])
         (set-box! box (max (unbox box) next-val))
       (hash-set! regs (line-reg line) next-val)
       regs))
     regs))))
...
(define inp (part1 example-input))
inp

(define real-input "n dec 271 if az < 3
f inc -978 if nm <= 9
g inc -336 if ga < 2
egk dec 437 if y < 5
z dec -550 if g == -336
